...
반대로 설치만 최신으로 받고 싶고 clone 관리는 하고 싶지 않다면, 업데이트하려는 실제 프로젝트 폴더에서 아래 한 줄을 실행한다. 최신 GitHub archive를 임시 폴더에 받은 뒤, 현재 폴더를 repo-level target으로 보고 다시 설치한다.
