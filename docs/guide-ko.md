...
installer 언어 값은 `auto`, `ko`, `en`, `ja`를 지원한다. `auto`는 한국어와 일본어 locale을 감지하고, 일본어로 강제하려면 `--language ja`를 넘기면 된다.
...
Oracle 브라우저 동작도 설치 때 바꿀 수 있다.

- `hidden`: 기본 설정. `--browser-hide-window`로 Oracle 브라우저를 숨기려 한다.
- `attach`: 이미 실행 중이고 DevTools 접근이 켜진 Chrome에 `--browser-attach-running`으로 붙는다. 별도 Oracle Chrome 창을 새로 띄우고 싶지 않을 때 쓴다.
- `visible`: 디버깅용으로 브라우저를 보이게 띄운다.
- `render`: 브라우저 자동화를 쓰지 않고 `--render --copy`로 수동 복사 흐름을 쓴다.
...
설치된 값은 각 `SKILL.md` 옆의 `oracle-consult.config.json`에 저장된다. 재설치 없이 이 파일의 `browserMode`만 직접 바꿔도 된다.
...
wizard가 처음에 언어를 묻고, 설치 범위와 설치 대상을 고르게 한다. 지원 언어는 한국어, 영어, 일본어다. no-git 최신 installer는 기본값으로 wizard를 건너뛰고, 명령을 실행한 현재 폴더를 설치 대상 repo로 사용한다.

일반 consult는 fresh-by-default로 다룬다. 이전 Oracle session은 복구가 필요할 때 한 번만 확인하고, 그 복구가 실패하면 같은 prompt로 새 consult를 시작해야 한다. 기존 session 이어하기를 기본 경로로 쓰지 않는다.
